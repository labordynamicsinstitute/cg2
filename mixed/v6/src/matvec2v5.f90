         ...
         ! Differs from matvec2v3 in that xd and xf are transposed
         ! X'D in xd(ncov,npers) - not used here
         ! X'F-X'DD'F in xf(ncov,nfirm) 
         ...
         ! The firm effects ncov+1:ncoef2 (ncoef2=ncov+nfirm)
         !
...
!        call DEGEMV(TRANS, M, N, ALPHA, A, LDA, X, INCX, BETA, Y, INCY)
         call dgemv('N', ncov,nfirm, alpha, xf, ncov, xin(ncov+1), 1, &
              beta, rout(1), 1)
         ...
         call dgemv('T', ncov, nfirm, alpha, xf, ncov, xin(1), 1, &
              beta, rout(ncov+1), 1)
